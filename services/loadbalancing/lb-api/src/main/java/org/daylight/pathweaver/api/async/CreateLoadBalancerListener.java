...
public class CreateLoadBalancerListener extends BaseListener {
    ...
    public void doOnMessage(final Message message) throws Exception {
        ...
        try {
            logger.debug(String.format("Creating load balancer '%d' via plugin...", lbid));
            getReverseProxyLoadBalancerService().createLoadBalancer(accountId, dbLoadBalancer);
            logger.debug("Successfully created a load balancer via plugin.");
        } catch (Exception e) {
            logger.error(e.getMessage());
            dbLoadBalancer.setStatus(ERROR);
            dbLoadBalancer.setCreatedOnPlugin(false);
            ...
            String alertDescription = String.format("An error occurred while creating loadbalancer '%d' via plugin.", dbLoadBalancer.getId());
            logger.error(alertDescription, e);
        ...
        dbLoadBalancer.setCreatedOnPlugin(true);

...
public class DeleteNodeListener extends BaseListener {
    ...
    public void doOnMessage(final Message message) throws Exception {
        ...
        try {
            logger.debug(String.format("Removing nodes '[%s]' from load balancer '%d' in Plugin...", nodesToDeleteIdString, msg.getLoadBalancerId()));
            getReverseProxyLoadBalancerService().deleteNodes(msg.getAccountId(), msg.getLoadBalancerId(), nodesToDelete);
            logger.debug(String.format("Successfully removed nodes '[%s]' from load balancer '%d' in Plugin.", nodesToDeleteIdString, msg.getLoadBalancerId()));
        } catch (Exception e) {

...
import org.daylight.pathweaver.datamodel.extensions.json.ExtensionWrapper;
import org.daylight.pathweaver.datamodel.extensions.json.ExtensionsWrapper;
...
public class ExtensionService {
    ...
    public String getExtensionAsXml(String alias) throws Exception {
        String enabledExtension = PluginConfiguration.getExtensionPrefix();
        String[] arr = alias.toLowerCase().split("-pathweaver-"); // eg. alias = "CTX-PATHWEATHER-AL";
        String prefix = arr[0];
    ...
    public String getExtensionAsJson(String alias) throws Exception {
        String enabledExtension = PluginConfiguration.getExtensionPrefix();
        String[] arr = alias.toLowerCase().split("-pathweaver-"); // eg. alias = "CTX-PATHWEAVER-AL";
        String prefix = arr[0];
        String extensionName = arr[1];
        if (!enabledExtension.equalsIgnoreCase(prefix)) {
            throw new BadRequestException("Extension " + alias + " not found");

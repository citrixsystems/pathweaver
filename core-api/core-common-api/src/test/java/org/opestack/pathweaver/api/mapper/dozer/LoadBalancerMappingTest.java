...
public class LoadBalancerMappingTest {
    ...
    public static class WhenMappingALoadBalancerFromApiToDomain extends MappingBase {
        private org.daylight.pathweaver.core.api.v1.LoadBalancer apiLoadBalancer;
        private org.daylight.pathweaver.service.domain.entity.LoadBalancer domainLoadBalancer;

        @Before
        public void setUp() throws Exception {
            apiLoadBalancer = StubFactory.createHydratedDataModelLoadBalancer();
            domainLoadBalancer = mapper.map(apiLoadBalancer, org.daylight.pathweaver.service.domain.entity.LoadBalancer.class);
        }
        ...
        public void should_map_the_node_list_across_the_two_load_balancers_and_the_properties_of_individual_nodes() {
            Assert.assertEquals(apiLoadBalancer.getNodes().getNodes().size(), domainLoadBalancer.getNodes().size());
        ...
        public void should_map_the_virtual_ips_across_the_two_load_balancers_and_the_properties_of_individual_nodes() {
            Assert.assertEquals(apiLoadBalancer.getVirtualIps().getVirtualIps().size(), domainLoadBalancer.getLoadBalancerJoinVipSet().size());
            ...
            for (LoadBalancerJoinVip loadBalancerJoinVip : domainLoadBalancer.getLoadBalancerJoinVipSet()) {
                ...
                Assert.assertEquals(org.daylight.pathweaver.service.domain.entity.VirtualIpType.PUBLIC, loadBalancerJoinVip.getVirtualIp().getVipType());
            }
        }
    ...
    public static class WhenMappingALoadBalancerFromDomainToApi extends MappingBase {
        ...
        public void setUp() {
            ...
            apiLoadBalancer = mapper.map(domainLoadBalancer, org.daylight.pathweaver.core.api.v1.LoadBalancer.class);
        }

        @Test
        public void should_not_fail_when_domain_loadBalancer_is_empty() {
            domainLoadBalancer = new org.daylight.pathweaver.service.domain.entity.LoadBalancer();
            try {
                apiLoadBalancer = mapper.map(domainLoadBalancer, org.daylight.pathweaver.core.api.v1.LoadBalancer.class);
            } catch (Exception e) {
        ...
        public void should_map_nodes_and_its_properties() {
            final List<org.daylight.pathweaver.core.api.v1.Node> list = apiLoadBalancer.getNodes().getNodes();
            Assert.assertEquals(domainLoadBalancer.getNodes().size(), list.size());

            for (org.daylight.pathweaver.core.api.v1.Node node : list) {
                if (!(node.getId() == 1 || node.getId() == 2)) {
                    Assert.fail();
                }
                if (!(node.getPort() == 80 || node.getPort() == 81)) {
        ...
        public void should_map_virtual_ips_across_two_loadbalancers() {
            final List<org.daylight.pathweaver.core.api.v1.VirtualIp> list = apiLoadBalancer.getVirtualIps().getVirtualIps();
            ...
            for (org.daylight.pathweaver.core.api.v1.VirtualIp virtualIp : list) {
        ...
        @Test
        public void should_map_health_monitor_and_its_properties() {
            org.daylight.pathweaver.core.api.v1.HealthMonitor healthMonitor = apiLoadBalancer.getHealthMonitor();
    ...
    public static class WhenMappingADomainLoadbalancerToASimpleDataModelLoadBalancer extends MappingBase {
        private org.daylight.pathweaver.service.domain.entity.LoadBalancer loadBalancer;
        private org.daylight.pathweaver.core.api.v1.LoadBalancer dataModelLb;

        @Before
        public void setUp() {
            loadBalancer = StubFactory.createHydratedDomainLoadBalancer();
            dataModelLb = mapper.map(loadBalancer, org.daylight.pathweaver.core.api.v1.LoadBalancer.class, "SIMPLE_LB");
        }
        ...
        public void shouldThrowExceptionWhenMappingFromDataModelToDomainModel() {
            mapper.map(dataModelLb, org.daylight.pathweaver.service.domain.entity.LoadBalancer.class, "SIMPLE_LB");
        }
        ...
        public void shouldNotFailWhenDomainLoadbalancerIsEmpty() {
            loadBalancer = new org.daylight.pathweaver.service.domain.entity.LoadBalancer();
            try {
                dataModelLb = mapper.map(loadBalancer, org.daylight.pathweaver.core.api.v1.LoadBalancer.class, "SIMPLE_LB");
            } catch (Exception e) {
                Assert.fail("Empty domain load balancer caused this exception");
